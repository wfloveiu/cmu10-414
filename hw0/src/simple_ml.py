...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(X.shape[0] // batch):
        ...
        G1 = np.multiply(np.where(Z1 >0, 1, Z1), np.matmul(G2, W2.transpose()))
        
        W1 -= lr/batch * np.matmul(x.transpose(),G1)
        W2 -= lr/batch * np.matmul(Z1.transpose(), G2)
