...
class NDArray:
    ...
    def is_compact(self):
        ...
        return (
            self._strides == self.compact_strides(self._shape) # compact计算在紧凑情况下的stride
    ...
    def broadcast_to(self, new_shape):
        ...
        """
        """
        old_shape:(3,1,5); old_stride:(5,5,1)
        new_shape(3,6,5), new_stride:(5,0,1)
        在第二个维度上复制，但是不能改变底层的存储，因此第二个维度上的stride就是0，即a[x][0][y]、a[x][1][y]访问的是同一个位置
        """
        if not all((ori == 1 or ori == new) for ori, new in zip(self._shape[::-1], new_shape[::-1])):
            raise RuntimeError
        new_stride = np.zeros(len(new_shape), dtype=int)
        
        new_stride = list(self._strides)
        if len(self._shape) != len(new_shape):
            new_stride = [0] * (len(new_shape) - len(self._shape)) + new_stride
    ...
    def __getitem__(self, idxs):
        ...
        for i in range(len(idxs)):
            new_stride.append(self.strides[i] * idxs[i].step)
            offset += self.strides[i] * idxs[i].start
            new_shape.append((idxs[i].stop - idxs[i].start + idxs[i].step - 1) // idxs[i].step)
        new_shape = tuple(new_shape)
        new_stride = tuple(new_stride)
        return self.make(new_shape, new_stride, offset=offset,handle=self._handle, device=self.device)
