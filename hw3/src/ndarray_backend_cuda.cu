...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if(gid < size)
  {
    ...
    {
      offset += idx % shape.data[i] * strides.data[i];  //找到在该维度的偏移量
      idx /= shape.data[i];
    }
    out[gid] = a[offset];
  }
...
__global__ void MatmulKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, uint32_t M, uint32_t N, uint32_t P)
{
  ...
  scalar_t sum = 0;
  for(size_t i=0; i<cnt; i++) // 遍历a的某一行和b某一列的TILE块，对应位置累加得到这个块的out
  {
    // copy data from global memeory to shared memory
    // out块中的thread和a、b块中的的idx_x, idx_y一一对应
    if((i * TILE + thread_y) < N) // 防越界
    {
      a_tile[thread_x][thread_y] = a[x * N + i * TILE + thread_y];
      /*
      先找到这个out块对应a和b中哪个块计算的：
      out块的块索引为(block_x, block_y)，因此是由a中block_x行中的块和b中block_y列中所有的块按块计算出的
      当前的计算的是行(列)其中的第i个块，这个块的块索引是: 行-(block_x, i)，记为A 列(i, block_y)
      以行中的块为例，它和a_tile[TILE][TILE]中的元素一一对应，进行copy时,out块中的每个thread从a中复制一个element到a_tile
      a_tile[thread_x][thread_y] = a[block_x * TILE + thread_x][i * TILE + thread_y]
      而block_x * TILE + thread_x 等于 block_x * blockDum.x + thread_x 等于x
      因此a_tile[thread_x][thread_y] = a[x][i * TILE + thread_y], 等式右边展开成一维形式就是a[x * N + i * TILE + thread_y]
      */
    }
    ...
    // 将线程块当作整体来计算矩阵乘法，此时的计算方式就是naive
    if (x < M && y < P) //部分线程可能不需要计算
    {
      for(size_t j=0; j<TILE; j++)
        if(i * TILE + j < N) //a和b的边缘块(右侧、下侧)无需全部计算
...
__global__ void ReduceMaxKernel(const scalar_t *a, scalar_t *out, size_t reduce_size, size_t size)
{
  ...
  if(gid < size)
  {
    size_t a_gid = gid * reduce_size;
    out[gid] = a[a_gid];
    for(size_t i=0; i<reduce_size; i++)
      out[gid] = out[gid] >= a[a_gid + i] ? out[gid] : a[a_gid + i];
  }
}
...
void ReduceMax(const CudaArray& a, CudaArray* out, size_t reduce_size) {
  ...
  ReduceMaxKernel<<<dims.grid, dims.block>>>(a.ptr, out->ptr, reduce_size, out->size);
...
void ReduceSum(const CudaArray& a, CudaArray* out, size_t reduce_size) {
   ...
   */
  CudaDims dims = CudaOneDim(out->size);
  ReduceSumKernel<<<dims.grid, dims.block>>>(a.ptr, out->ptr, reduce_size, out->size);
