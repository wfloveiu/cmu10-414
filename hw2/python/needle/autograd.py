...
class Tensor(Value):
    ...
    @staticmethod
    # 这里的op就是传过来的self，即算子类型;inputs是Value的,因为算子计算同时面向的是Tensor和普通数字的
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        if len(node.inputs) > 0:  # 常量输入也是node节点，但是没有梯度，因此不用计算 
            gradient = node.op.gradient(node.grad, node)
            # 2种情况，tulple 或者 tensor 
            if isinstance(gradient, tuple):
                for i, son in enumerate(node.inputs):
                    node_to_output_grads_list.setdefault(son, [])
                    node_to_output_grads_list[son].append(gradient[i])
            else:
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    """
    """
    有点奇怪的是，常理上来说node_list包含所有node，但print显示只有一个node，因此要考虑node还未添加到node_list中的情况
    """
    visited = dict()
    topo_order = []
    
    for u in node_list:
        if not visited.get(u, False): #不在visited中或者为False
